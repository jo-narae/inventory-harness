...
구체적인 상한값은 지금 임의로 정하지 않고 **H4 실습에서 실제 수정 횟수를 관찰한 뒤 결정한다.**
...
구체적인 상한값은 H4 실습에서 실제 수정 횟수를 관찰한 뒤 결정한다.
...
| 단계 목표 주요 산출물  |        |                                                                              |
| ------------- | ------ | ---------------------------------------------------------------------------- |
| H0            | 심판     | `npm run verify`, Architecture Check, 보호 경로 검사, 테스트 DB 준비 과정, GitHub Actions |
| H1            | 계약서    | 유지보수 Issue Template                                                          |
| H2            | 구현 루프  | Issue → 구현 → verify → 재시도                                                    |
| H3            | PR 게이트 | PR → 독립 CI 재검증 → AI Review → 사람 승인                                           |
| H4            | 실전 검증  | 실제 유지보수 Issue 투입 + 실험 로그                                                     |

가장 중요한 원칙은 다음이다.

> **H0를 건너뛰고 H2부터 만들지 않는다.**

AI를 반복시키기 전에 먼저 **AI의 결과를 판단할 방법부터 만든다.**
...
`BLOCKED`, `VERIFY_FAIL`, `CI_FAIL` 등은 작업 도중 발생하는 이벤트다.
...
### 2. 종료 조건은 가능한 한 실행 가능하게 만든다

"잘 동작한다"보다 테스트와 숫자로 확인할 수 있는 조건을 사용한다.

### 3. AI의 말을 완료 근거로 사용하지 않는다
