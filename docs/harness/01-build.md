...
각 단계의 **읽을 것**은 최소 권장 목록이 아니라 **그 단계에서 읽을 수 있는 전부**다.
목록 밖 문서가 필요하다고 판단되면 읽지 말고 멈춘다
([00-ssot.md](00-ssot.md) 3절 「범위가 부족할 때」).
...
읽을 것      04-loop.md 1~3절    시작점 · 종료 조건 처리 · 구현 루프와 상한
             ...
             01-build.md 4절     산출물이 놓이는 자리 (이 문서)
             gh issue view <전달받은 번호>    루프를 돌려볼 대상
