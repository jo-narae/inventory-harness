...
읽기 범위는 좁게 시작하고, **필요가 확인됐을 때만** 넓힌다.

- **현재 작업에 필요한 문서만 먼저 읽는다.** 위 표에서 해당 작업 영역에 지정된 원본이 출발점이다.
- **다른 문서는 두 경우에만 추가로 읽는다.**
  ① 지금 읽고 있는 문서가 그 문서를 명시적으로 참조할 때
  ② 판단에 필요한 정보가 부족하다고 확인됐을 때
  "관련이 있어 보인다"는 이유로 미리 읽지 않는다.
