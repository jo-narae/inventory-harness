...
H3에 착수하기 전에 **개발용 Issue 1건이 열려 있어야 한다.** 루프를 돌려볼 대상이 없으면 만든 것을 실행해 확인할 수 없다. 개발용과 측정용의 구분은 §28.0에 있다.
...
**착수 조건** — 이 루프를 개발하려면 개발용 Issue 1건이 이미 열려 있어야 한다. 측정용 Issue는 사용하지 않는다 (§28.0).
...
측정용   아래 Issue 1~3. H5에서 하네스의 성능을 재는 Issue
```
