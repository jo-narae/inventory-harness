...
다만 그 통로는 상시 열려 있지 않다. main은 PR과 `verify`로만 변경되도록 잠가 두고, 사람이 보호 경로를 갱신할 때만 Repository admin bypass를 열었다가 main 반영이 끝나면 다시 닫는다.

> **AI는 admin 자격증명을 사용하더라도 main에 직접 commit·push하지 않고, 그 bypass를 스스로 열거나 닫지도 않는다.**

기술적으로 가능한 것과 해도 되는 것은 다르다. main은 보호 경로 검사가 돌지 않는 자리이므로, AI가 여기에 쓸 수 있으면 앞의 보호 목록 전체가 우회된다. 기준을 세우는 자리는 사람의 자리다.

bypass가 열려 있는 동안에는 이 규칙을 기계가 강제하지 못한다 — 자격증명만으로는 사람과 AI를 구분할 수 없다. 04-loop.md 3절의 시도 기록과 같은 성격의 규약이고, 받치는 것도 같다.
