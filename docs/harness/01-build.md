...
| 단계 | 목표 | 읽을 문서 | 완료 조건 |
|---|---|---|---|
| H0 | SSOT 정책 수립 | `docs/08-harness.md` | `00-ssot.md`가 네 질문에 답한다 |
...
완료 조건    00-ssot.md가 다음 네 질문에 답한다
             ① 어떤 질문에 어떤 원본을 보는가
...
현재 프로젝트에는 역할이 다른 여러 문서가 존재한다. 모든 정보를 하나로 합치기보다 **영역별로 어떤 문서가 기준인지 명확하게 정의한다.** 네 질문의 답은 `00-ssot.md` 자신이 원본이다.
...
```text
H0  □ docs/harness/00-ssot.md 가 네 질문에 답한다
